...
import com.geostar.geosmarter.traffic.datagenerate.util.FileUtils;
import com.geostar.geosmarter.traffic.datagenerate.vo.VehicleLogVO;
...
public class DataGenerate implements DataLoad, Serializable{
	...
	private void loadVehicleLogDataFromFile(JavaSparkContext jsc, SQLContext sqlContext) {
		...
		Dataset<Row> vehicleLogDataFrame = sqlContext.createDataFrame(vehicleLogRDD, VehicleLogVO.class);
	...
	private void loadRoadMonitorAndCameraDataFromFile(JavaSparkContext jsc, SQLContext sqlContext) {
		...
		Dataset<Row> rowDataFrame = sqlContext.createDataFrame(vehicleLogVORDD, VehicleLogVO.class);
	...
	public Integer[] generateRoadIds(JavaSparkContext jsc, SQLContext sqlContext, boolean loadRoadMonitorAndCameraData) {
		...
		if (loadRoadMonitorAndCameraData) {
			...
			Dataset<Row> roadMonitorAndCameraDataFrame = sqlContext.createDataFrame(roadMonitorAndCameraRDD, roadMonitorAndCameraSctrucTyps);
	...
	public void generateData(JavaSparkContext jsc, SQLContext sqlContext) {
		...
		Dataset<Row> vehicleLogDataFrame = sqlContext.createDataFrame(vehicleLogRDD, vehicleLogStructType);
		vehicleLogDataFrame.registerTempTable(Common.T_VEHICLE_LOG);
